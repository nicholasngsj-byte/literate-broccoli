//+------------------------------------------------------------------+
//|  XAGUSD NY Breakout EA  —  Silver Edition                        |
//|                                                                  |
//|  Cloned from the XAUUSD NY Breakout EA and tuned for Silver.     |
//|                                                                  |
...
//|                                                                  |
//|  PIP DEFINITION FOR XAGUSD:                                      |
//|  1 pip = $0.01 price move (e.g. 30.25 → 30.26).               |
//|  InpStopLossPips=30 → $0.30 SL distance per oz.               |
//|  Silver trades ~$25-35/oz; typical NY range ≈ 20-50 pips.      |
//|                                                                  |
//|  NEWS FILTER:                                                    |
...
input double   InpOffsetPips         = 3.0;        // Offset above/below range for stop entry (pips; 1 pip = $0.01 for XAGUSD)
...
input double   InpStopLossPips       = 30.0;       // SL distance in pips (1 pip = $0.01 for XAGUSD)
...
input double   InpMaxLots            = 10.0;       // Hard lot cap per order (0 = no cap)

input bool     InpUseTrailing        = true;
input double   InpTrailStartPips     = 25.0;       // Profit in pips before trailing starts
input double   InpTrailStepPips      = 15.0;       // Trail distance behind current price
...
// because XAGUSD reacts most strongly to USD surprises (Fed, CPI, NFP, etc.).
...
input group "=== TECH ==="
input ulong    InpMagic              = 234567;      // Unique magic number for XAGUSD EA
input int      InpSlippagePoints     = 20;
input bool     InpDebugPrint         = false;

input group "=== BREAK-EVEN STOP ==="
input double   InpBreakEvenPips      = 15.0;       // Move SL to entry+buffer when profit reaches this many pips (0 = disabled)
input double   InpBreakEvenBuffer    = 2.0;         // Extra pips beyond entry for the BE SL (absorbs spread on exit)
...
//---------------- Pip helper ----------------
// XAGUSD: 1 pip = $0.01 (price moves from 30.25 → 30.26).
...
bool IsHighImpactNewsNearby()
{
   ...
   // XAGUSD is highly sensitive to USD macro surprises — USD events
   // drive the bulk of silver's directional moves during NY hours.
   // Change to "" to catch all countries (adds noise from minor releases).
...
bool PlaceBreakoutOrders()
{
   ...
   // Don't place BuyStop if price already blew past it — would trigger on retrace
   if(ask >= buyPrice)
   {
      if(InpDebugPrint) Print("SKIP BuyStop: ask ", ask, " already >= buyPrice ", buyPrice, " (breakout already occurred)");
      lotBuy = 0.0;
   }
   // Don't place SellStop if price already blew past it — would trigger on retrace
   if(bid <= sellPrice)
   {
      if(InpDebugPrint) Print("SKIP SellStop: bid ", bid, " already <= sellPrice ", sellPrice, " (breakout already occurred)");
      lotSell = 0.0;
   }
   ...
   if(lotBuy > 0.0)
   {
      ok1 = trade.BuyStop(lotBuy, buyPrice, _Symbol, slBuy, tpBuy,
                          ORDER_TIME_SPECIFIED, expiry, "XAG NY BuyStop");
      if(!ok1)
   ...
   if(lotSell > 0.0)
   {
      ok2 = trade.SellStop(lotSell, sellPrice, _Symbol, slSell, tpSell,
                           ORDER_TIME_SPECIFIED, expiry, "XAG NY SellStop");
      if(!ok2)
...
int OnInit()
{
   ...
   // Warn if the EA is attached to a non-silver symbol.
   string sym_upper = _Symbol;
   StringToUpper(sym_upper);
   if(StringFind(sym_upper, "XAG") < 0 && StringFind(sym_upper, "SILVER") < 0)
      Print("WARNING: This EA is tuned for XAGUSD. Current symbol is ", _Symbol,
            ". Pip size and defaults may not be optimal.");
   ...
   Print("=== XAGUSD NY Breakout EA initialised ===");
   Print("Symbol: ", _Symbol, "  PipSize: ", DoubleToString(PipSize(), 4));
